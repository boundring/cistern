...
list), and no line exceeds 95 cols."
  ...
  (let ((src (format "%S" (symbol-function 'cistern--help-text))))
    (dolist (entry cistern--tile-table)
      (let ((g (cistern--tile-glyph (car entry))))
        ;; only the non-ASCII glyph set is probed: ASCII glyphs (?, !, +)
        ;; are ordinary text characters and may appear in prose
        (when (> (string-to-char g) 127)
          (cl-assert (not (string-match-p (regexp-quote g) src))
                     t "hardcoded tile glyph %s in the help builder" g)))))
...
no banner and no drain."
  ...
  (let ((st (cistern--new-game 20260830)))
    ...
    (let* ((story (cistern-st-story st))
      ...
      (plist-put creak :state 'resolved)
      (plist-put creak :resolved-as 'pass)
      ;; pin the tier draw to land COMMON (the tree's :tier)
      (let ((probe 1))
        (while (/= (mod (ash (cistern--stream-next probe) -6) 100) 0)
          (setq probe (1+ probe)))
        (plist-put story :roll-pos probe))
...
pickup banks alloy and logs."
  ...
  (cistern--story-apply-effect st (list :effect 'tile-place :arg 'cache)
                               (cons 8 6))
  (cl-assert (cl-some (lambda (tile) (eq (cistern--cell st (car (car tile))
                                                  (cdr (car tile))) 'event))
                      (cistern-st-event-tiles st))
             t "tile-place did not spawn an event tile")
  (cl-assert (= (length (cistern-st-event-tiles st)) 1)
             t "event tile count wrong")
  (cistern--story-apply-effect st (list :effect 'tile-place :arg 'cache)
                               (cons 0 0))
  (cl-assert (eq (cistern--cell st 0 0) 'wall) t "tile on a wall")
  ;; silent countdown: decays per tick, no logs, expiry clears
  (setq tile-cell (car (car (cistern-st-event-tiles st))))
  (setq logs0 (length (cistern-st-log st)))
  (cistern--sim-tick st)
  (cistern--sim-tick st)
  (cl-assert (eq (cistern--cell st (car tile-cell) (cdr tile-cell)) 'event)
             t "expired early")
  (cl-assert (= (length (cistern-st-log st)) logs0) t "the countdown logged")
  (cistern--sim-tick st)
  (cistern--sim-tick st)
  (cl-assert (eq (cistern--cell st (car tile-cell) (cdr tile-cell)) 'floor)
             t "expiry did not clear")
...
dialogue tree selection by tier weights + drift with once gates."
  ...
  (let ((st (cistern--new-game 20260830)))
    ...
    (let* ((story (cistern-st-story st))
      ...
      (plist-put creak :state 'resolved)
      (plist-put creak :resolved-as 'pass)
      ;; pin :roll-pos so the tree-selection tier draw lands COMMON
      (let ((probe 1))
        (while (/= (mod (ash (cistern--stream-next probe) -6) 100) 0)
          (setq probe (1+ probe)))
        (plist-put story :roll-pos probe))
      (let* ((before (plist-get story :roll-pos))
        ...
        ;; drain the queued conversation lines (one per tick, D5)
        (dotimes (_ 4) (cistern--dialogue-eval st))
        ;; cooldown: with the queue drained and the tick unchanged,
        ;; no new tree opens (60-tick cooldown vs 25-tick elapsed)
        (let* ((after (plist-get story :roll-pos))
               (out2 (cistern--dialogue-eval st)))
          (cl-assert (= (length (cdr out2)) 0)
                     t "cooldown ignored")
          (cl-assert (> after before)
                     t "stream-2 did not advance (tier + branch rolls)")))))
...
tick-for-tick vs a no-narrative run, and the five soars re-probe."
  ...
  (setq cistern--banks nil cistern--story-copy nil)
  (setq sa (cistern--new-game 20260830))
  (dotimes (_ 50) (cistern--do-tick sa))
  (setq rng-a (cistern-st-rng sa) prng-a (cistern-st-particle-rng sa))
  (setq cistern--banks nil cistern--story-copy nil
        cistern--matrix-sources (make-hash-table :test 'eq))
  (cistern--banks-load
   (list (expand-file-name "data/banks/example.el"
                           cistern-test-v4--root)))
  (setq sb (cistern--new-game 20260830))
  (dotimes (_ 50) (cistern--do-tick sb))
  (setq rng-b (cistern-st-rng sb) prng-b (cistern-st-particle-rng sb))
  (cl-assert (= rng-a rng-b) t "sim LCG diverged with narrative")
  ;; the particle stream is rewards-driven: the story's goal-mod
  ;; legitimately shifts sparkle pacing - the guard is that story code
  ;; never DRAWS it (static check) and the sim LCG is tick-for-tick
  (cl-assert (= rng-a rng-b) t "sim LCG diverged with narrative")
