  ...
  (let ((st (cistern--new-game 42)))
    (let* ((before (split-string (cistern-view--render st) "\n"))
      ...
      (let ((after (split-string (cistern-view--render st) "\n")))
        ...
        (cl-assert (= (length (nth 10 after)) (length (nth 10 before)))
                   t "flood changed its own row length")
        (dotimes (y before-rows)
          (unless (= y 10)
            (cl-assert (string= (nth y before) (nth y after))
                       t "flood pop-in shifted row %d" y)))))))
  ...
  (let ((st (cistern--new-game 42)))
    ...
    (let ((w (car (cistern-st-creators st))))
      ...
      (cl-assert (null (cistern-st-rewards-events st))
                 t "manifold relief paid tank income")))
