...
migration (arming is a use-case call)."
  ...
  (let* ((st (cistern--new-game 42))
         (lay (cistern-view--layout 200 60
                                    (cistern-st-w st) (cistern-st-h st)))
         (lay-badge (cistern-view--layout 200 60
                                          (cistern-st-w st) (cistern-st-h st)
                                          nil t)))
    (cl-assert (equal (cistern-view--cell-at st lay 4 0) '(0 . 0))
               t "first map line/col is cell (0,0)")
    (cl-assert (equal (cistern-view--cell-at st lay 4 17) '(17 . 0))
               t "column maps to x")
    (cl-assert (equal (cistern-view--cell-at st lay 12 5) '(5 . 8))
               t "line maps to y past the header")
    (cl-assert (null (cistern-view--cell-at st lay 1 0))
               t "header lines are outside the map")
    (cl-assert (null (cistern-view--cell-at st lay 3 0))
               t "last header line is outside the map")
    (cl-assert (null (cistern-view--cell-at
                      st lay (+ 4 (cistern-st-h st)) 0))
               t "past the last map row is outside the map")
    (cl-assert (null (cistern-view--cell-at st lay 4 (cistern-st-w st)))
               t "past the last map column is outside the map")
    ;; badge live: the map starts one line lower (R2-Q02 block height)
    (cl-assert (equal (cistern-view--cell-at st lay-badge 5 0) '(0 . 0))
               t "badge variant: origin tracks the reserved row")))
