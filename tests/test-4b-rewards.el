  ...
  (dolist (case '((pipe 6 2 2 1) (toilet 6 3 10 5) (tank 6 4 15 7)))
    (let* ((kind (nth 0 case))
      ...
      (let* ((rng0 (cistern-st-rng st))
        ...
        (dolist (p ps)
          (cl-assert (equal (plist-get p :pos) (cons x y))
                     "dust spawns at the demolished tile")
          (cl-assert (member (plist-get p :glyph) cistern-test-4b-m1-dust-glyphs)
                     "dust glyphs from the M1 row")
          (cl-assert (eq (plist-get p :face) 'info)
                     "dust face is the neutral palette enum")
          (cl-assert (eq (plist-get p :layer) 'sparkle)
                     "dust layer is sparkle")
          (cl-assert (and (>= (plist-get p :ttl) 2) (<= (plist-get p :ttl) 3))
                     "dust ttl 2-3 from the M1 row")
          (cl-assert (and (<= (abs (car (plist-get p :vel))) 1)
                          (<= (abs (cdr (plist-get p :vel))) 1))
                     "dust vel is short"))
        (cl-assert (= (cistern-st-rng st) rng0)
...
(defun cistern-test-4b-m4-reputation ()
  ...
  ;; events, spawns dust into the field, the render shows it and does
  ;; NOT re-drain (two renders byte-identical)
  (let ((st (cistern--new-game 42)))
    (cistern--cmd-demolish st 4 2)          ; starter pipe (player-equivalent)
    (cistern--do-tick st)
    (cl-assert (null (cistern-st-rewards-events st))
               nil "the tick drained the pending events")
    (cl-assert (cistern-st-particles st)
               nil "dust particles spawned into the field by the tick")
    (let* ((row (nth (+ 3 2) (split-string (cistern-view--render st) "\n")))
      ...
      (cl-assert (string= row row-again)
                 nil "render reads the field, never re-evaluates")))
...
(defun cistern-test-4b-m5-relieve-pay ()
  ...
  (let ((st (cistern--new-game 42))
    ...
    ;; 80 popups exceed K=64: the FIFO keeps the newest 64 (L-029)
  ...
  (let ((st (cistern--new-game 42)))
    ...
    (let ((p (car (cistern-st-particles st))))
      (cl-assert (and p (eq (plist-get p :layer) 'popup))
                 nil "one popup particle per paying relief")
      (cl-assert (equal (plist-get p :pos) (cons 12 6))
                   nil "popup spawns at the relief tile")
        (cl-assert (string= (plist-get p :glyph)
                            (format "+%d" cistern-score-relief-base))
                   nil "popup glyph is +N")
        (cl-assert (= (plist-get p :ttl) 3) nil "popup ttl 3 (§4 row)")
        (cl-assert (equal (plist-get p :vel) (cons 0 -1))
                   nil "popup vel drifts up (§4 row)")
        (cl-assert (memq (plist-get p :face) '(success bonus))
...
(defun cistern-test-4b-m6-particle-field ()
  ...
  (let ((fields nil))
    (dolist (seed '(1 2 3 4 5))
      (let ((st (cistern--new-game seed)))
        (cistern--cmd-demolish st 4 2)
        (cistern--rewards-eval st nil)
        (push (secure-hash 'md5 (prin1-to-string (cistern-st-particles st)))
              fields)))
  ...
  (let ((st (cistern--new-game 42)))
    ...
    (cl-assert (null (cistern-st-particles st))
               nil "ttl-6 particle removed at ttl 0 on the 6th advance")
    (cistern--advance-particles st)
    (cl-assert (null (cistern-st-particles st))
...
(defun cistern-test-4b-m7-severity ()
  ...
  (let ((st (cistern--new-game 42)))
    ...
    (let* ((breach (cl-find-if (lambda (e) (string-match-p "BREACH" (car e)))
                               (cistern-st-log st)))
           (breach-line (car breach))
           (intent (cl-find-if (lambda (i)
                                 (and (eq (plist-get i :layer) 'log)
                                      (equal (plist-get i :text) breach-line)))
  ...
  (let ((st (cistern--new-game 42)))
    ...
    (let* ((relief-line (cl-find-if
                         (lambda (e) (string-match-p "RELIEVED" (car e)))
                         (cistern-st-log st)))
           (relief-text (car relief-line))
           (intent (cl-find-if (lambda (i)
                                 (and (eq (plist-get i :layer) 'log)
                                      (equal (plist-get i :text) relief-text)))
                               (cdr (cistern-st-rewards-outcome st)))))
...
(defun cistern-test-4b-m9-ceremony ()
  ...
  (let ((st (cistern--new-game 42)))
    ...
    (cistern--rewards-eval st (list (list 'relief 70 3 3)))
    (let ((ps (cistern-st-particles st)))
      ;; the relief's ttl-3 popup coexists in the buffer; the ceremony
      ;; fill is the ttl-6 sparkle set
      (let ((sparkles (cl-remove-if-not
                       (lambda (p) (eq (plist-get p :layer) 'sparkle)) ps)))
        (cl-assert (and (>= (length sparkles) 60) (<= (length sparkles) 64))
                   nil "ceremony fills the buffer (up to 64)")
        (dolist (p sparkles)
          (cl-assert (= (plist-get p :ttl) 6) nil "ceremony ttl 6")
          (cl-assert (equal (plist-get p :vel) (cons 0 0))
                     nil "ceremony sparkles static")
