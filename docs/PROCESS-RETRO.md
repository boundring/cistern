...
## Handoff-phase lessons — v6 halt + Hngh queue (2026-09-08)

Evidence: the v6 wave-1 gentle halt (impl-v6w1 halted at its own green boundary; V6-01/02 landed d059270/beb3fff, V6-03 pending with recovery block L-112, suite 140/140, tree clean at d638e3f) and the Hngh intake queue build.

1. **HALT AT A GREEN BOUNDARY, WITH A VERBATIM RECOVERY BLOCK.** The halt directive said 'clean boundary beats a fuller batch' — the run committed V6-01/02, captured byte-identity anchors (sha256 of full state per seed) BEFORE the next run changes generation defaults, wrote the V6-03 recovery instructions verbatim-executable into L-112 (not a summary — the actual draw-order spec, theme knobs, tooling notes), and synced the install. A replacement can resume with zero re-discovery. Rule: every handoff report carries a recovery block someone could execute without reading the transcript.
2. **CAPTURE PROOF ANCHORS BEFORE GENERATION-AFFECTING CHANGES.** The halt run recorded per-seed state hashes pre-V6-03 so the next run can prove the size/theme knobs add zero LCG draws on standard maps. Rule: before any change that could alter generative output, snapshot the anchors and hand them to the successor as the regression test.
3. **PEER COORDINATION BETWEEN PARALLEL DESIGNERS WORKS WITHOUT A DIRECTOR.** Four v6 designers allocated streams 4-6, tile tables, and copy namespaces via their own hub sessions; the conflicts the director ruled were genuine ambiguities, not coordination failures. Rule: brief parallel designers with each other's existence and a hub channel; rule only the genuine collisions.
4. **THE TTSR RULES APPLY TO THE DIRECTOR TOO.** The director's own todo bookkeeping was caught three times by todo-call-missing-op/todo-init-requires-phase. The rule set is not worker-only.
5. **NOT EVERYTHING IS TTSR-EXPRESSIBLE — AND THAT IS FINE.** The stall analysis routed genuinely: giant writes → TTSR (stream-regex catchable), provider aborts → process rules (commit-per-green + replacement briefs), silent-wrong-answer → always-apply probes. Forcing every lesson into a stream rule would have made the rules noisy without making the work safer.
